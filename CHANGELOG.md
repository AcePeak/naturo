...
### Added
- **Ergonomic in-process Python SDK** — `from naturo import see, find, click, type, press, hotkey, scroll, get_value, set_value, capture, launch, quit, windows, monitors`, plus a `Desktop`/`App` object API, so naturo is usable as a library without shelling out to the CLI ([#924](https://github.com/AcePeak/naturo/issues/924)).
- **`naturo run script.py` / `naturo run -c "code"`** — execute user Python scripts under the resolved interpreter (system-preferred, embedded fallback) with the SDK importable in-script; supports `--args` (→ `sys.argv`), `--timeout` (kills the whole process tree), distinct file/syntax/import/runtime error reporting, and verbatim exit-code propagation ([#42](https://github.com/AcePeak/naturo/issues/42)).
- **Embedded Python 3.12 runtime bundler** (`scripts/bundle_python.py`) + a system/embedded interpreter resolver — the foundation for zero-Python distribution ([#41](https://github.com/AcePeak/naturo/issues/41)).
- **Fully self-contained `.mcpb` bundle** (`build_mcpb.py --self-contained`) — vendors the embedded runtime + `naturo_core.dll` so the Claude Desktop extension installs with no prior `pip install naturo` ([#997](https://github.com/AcePeak/naturo/issues/997)).
- **Standalone `naturo.exe`** via PyInstaller (`scripts/build_exe.py` + `naturo.spec`) — a single-file executable that runs with no system Python ([#43](https://github.com/AcePeak/naturo/issues/43)).
- **Win32 API hooking** (MinHook, vendored) — in-process log/block hooks via a thread-safe hook manager, exposed as `naturo hook install/list/remove/monitor` (CLI) and MCP hook tools; cross-process DLL injection is admin-gated and CLI-only, not an MCP tool ([#40](https://github.com/AcePeak/naturo/issues/40)).
- **`excel create-chart`** on CLI + MCP (`excel_create_chart`) — bar/column/line/pie/area/scatter charts via the COM object model ([#1319](https://github.com/AcePeak/naturo/issues/1319)).
- **UIA accessibility properties in the element schema** — `is_enabled`, `is_offscreen`, `help_text`, `localized_control_type`, `is_keyboard_focusable`, `has_keyboard_focus`, emitted by the native tree walk ([#896](https://github.com/AcePeak/naturo/issues/896)).
- **Official MCP registry manifest** (`server.json`) so naturo can be published to registry.modelcontextprotocol.io ([#928](https://github.com/AcePeak/naturo/issues/928)).
- **Latency benchmark harness** (`benchmarks/latency/`) — percentile/regression tracking for see/find latency against an offline fixture ([#418](https://github.com/AcePeak/naturo/issues/418)).
- **Scriptable `--field` selection** for `list`/`app` commands, and the complete window-target flag matrix (`get`/`set --pid`, `app quit --window`/`--hwnd`) ([#1206](https://github.com/AcePeak/naturo/issues/1206), [#871](https://github.com/AcePeak/naturo/issues/871)).

### Documentation
- Tutorial series (automate Notepad, Excel, and build an AI agent) ([#415](https://github.com/AcePeak/naturo/issues/415)), a "How Naturo Works" architecture deep-dive ([#56](https://github.com/AcePeak/naturo/issues/56)), a CJK/Chinese-desktop coverage note ([#921](https://github.com/AcePeak/naturo/issues/921)), an API-stability & SemVer policy ([#92](https://github.com/AcePeak/naturo/issues/92)), a release-process guide ([#925](https://github.com/AcePeak/naturo/issues/925)), and populated recognition/supported-apps matrices ([#937](https://github.com/AcePeak/naturo/issues/937), [#421](https://github.com/AcePeak/naturo/issues/421)).

### Fixed
- **Single code path for single-element value read; `see` and `get` now agree** — collapsed the two stacked readers into one method that `see`-population and `get` share, so `see` populates Edit/Document values consistently with `get` (verified byte-for-byte on live Notepad) ([#1212](https://github.com/AcePeak/naturo/issues/1212)).
- **Raw-view fallback for ScrollViewer-wrapped multiline edit values** — when a control exposes only a ScrollViewer (50033) whose patterns read empty, both readers now walk the raw UIA view for the inner Edit/Document; strictly additive ([#1213](https://github.com/AcePeak/naturo/issues/1213)).
- **`quit` verifies by window-ownership before reporting success** — no longer reports success while a single-instance app (e.g. Win11 Notepad) is still running ([#1197](https://github.com/AcePeak/naturo/issues/1197)).
- **`naturo type` default is now the IME-immune ValuePattern→clipboard→keystroke ladder (fixes CJK/TSF corruption)** — the CLI `naturo type` default path called the raw keystroke backend (`SendInput`) directly, which modern CJK/TSF IMEs (e.g. Win11 Notepad under Microsoft Pinyin, HKL 0x08040804) intercept and corrupt (`naturo`→`nature`, `quick`→`cccck`), while MCP `type_text` already climbed a reliable ladder. Both surfaces now route through one shared helper, `naturo.actions.smart_type_text`, so the default `naturo type` (with `--input-mode normal` and no `--paste`) tries a writable UIA **ValuePattern** first (instant, no keyboard, IME-immune), then an atomic **clipboard paste** (also IME-immune; what heavy TSF controls like Notepad need), and only falls back to **keystroke** injection as a last resort. Multiline text skips the ValuePattern rung (UIA `SetValue` collapses newlines, #563) and is pasted atomically, preserving the exact string. The clipboard rung saves and restores the user's prior clipboard (honoring `--restore`, default on). The delivered rung is reported as `input_method` (`value_pattern`|`clipboard_paste`|`keystroke`) in the result, mirroring MCP. Explicit escape hatches are unchanged: `--paste` forces clipboard, and `--input-mode hardware`/`hook` force raw keystroke injection, bypassing the ladder ([#1219](https://github.com/AcePeak/naturo/issues/1219))
- **Repo-wide `-j`/`--json` success-envelope sweep + a universal self-maintaining guard** — completes the umbrella [#1142](https://github.com/AcePeak/naturo/issues/1142) that the per-subcommand fixes (#865/#1054/#977/#1141/#876) kept re-opening one command at a time. Audited **every** json-emitting CLI subcommand (140 total, whole Click tree) and normalized every remaining success path that omitted the top-level `success` key. **JSON-shape changes** for downstream consumers (each gains a leading `"success": true`; all previously-present keys are unchanged unless noted): the entire **`browser`** command family — `navigate`, `url`, `title`, `eval`, `tabs` (was a **bare array** → now `{success, tabs, count}`), `scroll`, `find` (`--all` was a bare `{elements, count}`), `text`, `attr`, `html`, `click`, `type`, `select`, `hover`, `frames` (was bare `{frames, count}`), `frame-eval`, `frame-find`, `wait`, `wait-navigation`, `wait-url`, `wait-function`, `wait-network-idle`, `screenshot`, `captcha-detect`, `profiles`, `stealth-flags` — several of which previously emitted `{"status": "ok", ...}` (the `status:ok` marker is replaced by `success:true`); plus `record show -j` (was a bare `to_dict()`) and `selector export -j` piped to stdout (was a bare re-importable document → now `{success, app, selectors, count}`; the `--output <file>` document written to disk is unchanged). A new hermetic test, `tests/test_json_envelope_sweep_1142.py`, introspects the live Click tree and asserts every json-emitting subcommand is either **runtime-driven** (empty stores / mocked browser page / pure compute — its emitted JSON asserted to carry a top-level boolean `success`) or in an **explicit, reason-annotated allow-list** for commands that need a live desktop/DLL, CDP browser, or Excel COM to reach their success path — so a future subcommand that forgets the envelope, or is added without being classified, fails CI immediately. Ends the drip ([#1142](https://github.com/AcePeak/naturo/issues/1142))
- **CLI `-j` success envelope made uniform across `see`/`get`/`set` + `browser close`/`tab`, and MCP `isError` now tracks payload `success`** — closes a cluster of JSON-contract drift where a command's `-j` **success** path used a different top-level shape than its **error** path, forcing scripters/agents to detect success with `has("error")` instead of branching on `.success`. **BREAKING (`see -j`):** `naturo see -j` now emits `{"success": true, "tree": {...}, "snapshot_id": ..., "cascade_stats": ..., "recognition_summary": ..., "dpi_context": ...}` instead of a bare element tree at the root — the element tree moved under `"tree"` and the run-level metadata sits alongside it (a script reading `see -j | jq '.role'` must now read `.tree.role`) ([#865](https://github.com/AcePeak/naturo/issues/865)). `naturo get -j` (single element) now leads with `"success": true` before the element fields, and `get --all -j` returns the canonical `{"success": true, "elements": [...], "count": N}` collection envelope instead of a bare array; `set -j` already carried `"success": true` ([#1054](https://github.com/AcePeak/naturo/issues/1054)). `naturo browser close` and `browser tab` — the only 2 of 35 `browser` subcommands lacking `--json` — now accept `--json/-j` and emit `{"success": true, "action": "browser_close"|"browser_tab", ...}` (and route connection failures through the standard error envelope), matching their siblings ([#1152](https://github.com/AcePeak/naturo/issues/1152)). On the MCP surface, the transport-level `result.isError` is now derived from the tool payload so `isError == not payload.success` for every tool result: a tool that returns `{"success": false, ...}` (validation/automation/runtime failure) now also arrives with `isError: true`, giving agents a single discriminator instead of having to check both `isError` and `payload.success`; genuine protocol failures (unknown tool, malformed arguments, no desktop session) still surface as `isError: true` with no payload, unchanged ([#882](https://github.com/AcePeak/naturo/issues/882))
...
### Fixed
- **Failed `capture --region`/`--element` no longer leaves the full uncropped screenshot on disk** — the backend writes the *full* screenshot to the user's `-o`/`--path` file before the crop is validated, so a crop-validation failure (non-positive `--region` W/H, fully off-screen region/element, zero-size element, stale `--element` ref) returned `success: false` / exit 1 **but left the full-screen image behind**, contradicting the reported failure (a never-lie defect — an agent that ignored the exit code consumed the wrong full-desktop image, plus disk litter and a privacy surprise where a region crop is often requested precisely to avoid persisting the whole desktop). When a crop is requested, the pre-written file is now removed best-effort on every validation-failure path before the error is raised; a successful crop and a plain no-crop capture both keep their file unchanged. Also fixes a latent bug where the zero-size-element message read `element.name` (a non-existent attribute) — the resulting `AttributeError` was swallowed by the broad handler and mis-reported as `CAPTURE_ERROR` instead of `ZERO_SIZE_ELEMENT`; it now reads the real `title`/`label` fields ([#1114](https://github.com/AcePeak/naturo/issues/1114))
