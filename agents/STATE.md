...
>
> Last refreshed: 2026-07-02 (Orc autonomous cycle — **PAUSE LIFTED; the loop resumed under the NEW `/goal`-session process — `agents/GOAL.md` is now the single source of truth, NOT this file's old issue-milestone loop.** develop CI **GREEN** at HEAD `0a265e6`. Since the 07-01 pause: the `/goal` session shipped **M1→M2→M3 in one day** and advanced to M4 — M1 Unified Auto Element Tree (PR #1214); M2 broadened coverage COM/Excel + local OCR + JAB, published `docs/SOFTWARE_ADAPTATION.md` (#1215/#1216/#1217); M3 AI-agent fit MCP + self-correcting error contract + 3-line integration doc + agent-in-the-loop (#1218), plus an enterprise comtypes gen-cache self-heal (#1219/#1220). **CURRENT MILESTONE = M4 Reliability & soak** (no silent failures / crash-recovery generalized / zero process leaks / ≥100-cycle soak on ≥2 real apps / CI-runnable). **Dangling-PR triage (pre-pivot, forgotten during the pause):** #1194 (wildcard-host selector round-trip, fixes #1190) MERGEABLE+CLEAN+**all CI green** since 06-29 → ready to land, first reconcile candidate; #1203 (JAB states, part of #1200) **still RED** (Python-Tests FAIL; one-line root cause `getattr(el,'states',None)` in `_tree.py`) — note M2 already added JAB to the fused tree, so possibly part-superseded; #1170 (find --ocr, fixes #1060) CONFLICTING/DIRTY and M2 shipped local OCR → rebase-or-close; #1167 (dependabot checkout 7) targets `main`, only Feishu-notify fails (noise); #1055 malformed (head==base==main) → close. **KEY STRATEGIC GAP for Ace:** the `/goal` process (GOAL.md, M1–M4, direct PRs #1214–#1220) has **diverged from the GitHub issue/milestone tracker** — 88 open issues across v0.3.2–v0.5.0 + these 5 dangling PRs are untouched by /goal. Decide: reconcile (land green #1194, rebase/close the rest, archive the stale version-milestones) OR formally retire the version-milestone tracker in favor of GOAL.md. I did **not** unilaterally merge/close (prior #1203 caution precedent) — surfaced for decision. **Risks:** process-artifact drift — the body of THIS file (Operating Mode = local loop, Current Version v0.3.1) describes the superseded process; **GOAL.md is authoritative**. stars 5 vs 1K (distribution is an explicit later phase per GOAL). Evidence: `.work/reviews/2026-07-02-0008-daily-review.md`.)
>
