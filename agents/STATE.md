...
>
> Last refreshed: 2026-07-04 (Orc autonomous cycle — `/goal`-session process, **`agents/GOAL.md` is the single source of truth**. develop CI **GREEN**. **Engineering phase M1–M4 all DONE** (07-01, PRs #1214–#1231). Project flipped **2026-07-02 to PHASE 2 — DISTRIBUTION**; **CURRENT MILESTONE = D1** (public, reproducible competitive coverage matrix vs OSS rivals). **D1 ≈ 50%:** harness + matrix-logic test landed (#1245/ex-#1233, 9/9 green, Linux-collectable, no numbers hand-authored) — criteria #1/#4 met; **criteria #2/#3 (publish `docs/COMPETITIVE.md` FROM REAL rival runs) are ENV-BLOCKED** — they need a real Windows-desktop session and no self-hosted Windows runner is online (ref closed #842). **This has been the blocker across ≥2 cycles → escalated to Ace:** either (1) bring a Windows desktop/runner online (or Ace runs `python -m benchmarks.competitive.run_competitive --markdown` on the fixtures and drops in the output) so a round regenerates the matrix + QA re-verifies, OR (2) mark un-runnable rival cells `blocked: needs env` honestly and advance the queue. Meanwhile the loop ships unblockable pillar-B/agent-fit polish (#1238–#1254: MCP hwnd targeting, recovery-hints, launch-latency, `launch_browser`, `read_web_text` via CDP, Excel COM over MCP) — good, but off D1's gating step. **Open PRs:** only #1203 (JAB states) — behind/RED/auto-merge-stuck, commented 07-03 for rebase-or-retire. **Legacy trackers dormant:** the GitHub issue/milestone tracker (88 stale issues) + Dev-Sirius github-queue (idle since 04-05, no pending requests) are NOT driving work; reconcile-or-retire remains a pending Ace decision. **Phase-2 human gate:** README/PyPI/"we beat X" claims open auto-merge OFF for Ace; engineering PRs still auto-merge on green. **Note:** the structural body of THIS file predates the pivot — GOAL.md is authoritative. Evidence: `.work/reviews/2026-07-04-daily-review.md`.)
>
