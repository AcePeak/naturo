...
- [TC-0085](exploratory/mcp-window-target-param-drift.yaml) — P2: MCP window-targeting param name drifts across surfaces — `focus_window` uses `title`, `see_ui_tree`/`capture_window`/`wait_for_*`/`create_snapshot`/`menu_inspect` use `window_title`, CLI uses `--window`; #891 turns the wrong-name case into silent fallback to "foreground" (#900) — **active**
