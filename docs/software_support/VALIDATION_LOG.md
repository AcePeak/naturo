...
| 40 | Windows 画图 (Paint, hwnd 268682) | Win11 WinUI XAML (MSPaintApp) | 2026-08-01 | ✅ full WinUI toolbar/tools by ref (124 nodes) | ✅ tools by ref | det (UIA) | **SUPPORTED — deep modern-WinUI coverage.** `--depth 9` exposes the whole ribbon/tool surface by ref: toolbar 保存/共享/撤消/重做/打开设置; 工具 group 铅笔/填充/文本/橡皮擦/颜色选取器/放大镜; 图像 group 选择/裁剪/删除背景/旋转/翻转/重设大小; 画笔 + 形状 (形状轮廓/形状填充) — 124 nodes total. Core features 免登录 (optional MS sign-in button present but not required). ⚠️ **naturo finding (for follow-up):** `naturo app launch mspaint` **by bare name blocked >120s** — `mspaint` is on PATH so it went through `cmd /c start /wait`, which blocks until the GUI app closes (Paint persists, unlike calc.exe whose stub exits fast). Workaround: launch by `--path C:\Windows\System32\mspaint.exe` (non-blocking Popen). The `start /wait` was likely added for UWP PID resolution; a safe fix is to use non-blocking Popen with the `where`-resolved path for on-PATH GUI apps. Already-present system app; market-download stall. |
...
**Installed + evaluated this run: 26 apps (#12–31, #33, #36–40; #34 Greenshot &
#35 HWiNFO64 PARTIAL; +#32 DB Browser installed but its stale 2015 build crashes
on launch — a failure).** #36–40 (Notepad++, Calculator, Task Manager, Registry Editor,
Paint) were evaluated from already-present installs during a 电脑管家 download stall (see Open items). Wide tech coverage, mostly
