...
| 53 | Inkscape 1.0.2 (免登录, hwnd 4135704) | GTK (gdkWindowToplevel) | 2026-08-01 | ⚠️ window only; GTK content a11y-blind | ⚠️ OCR/coords | partial | **PARTIAL — GTK-on-Windows is a11y-blind (new toolkit data point).** 电脑管家-acquired install (`inkscape-1.0.2-2-x64.msi`, 130MB) — installed via the **elevated-MSI fix** (#52) + naturo-driven wizard (license checkbox e3 → Next → Install → Finish). Vector editor; the GTK window (`gdkWindowToplevel`) exposes only **7 UIA / 1 MSAA node** (chrome only) — GTK's menus/toolbars/canvas don't bridge to Windows UIA or MSAA (parallel to the a11y-less Qt cmake-gui #29). Operation needs OCR/vision. Confirms naturo's toolkit map: **GTK apps require the OCR/AI fallback** — the moat's non-a11y path. |
| 54 | Windows 字符映射表 (charmap, hwnd 726374) | Win32 (#32770) UIA | 2026-08-01 | ✅ font combo + copy controls | ✅ by ref | det (UIA) | **SUPPORTED — classic Win32.** UIA exposes 字体 ComboBox (e11), 复制字符 Edit (e4), 选择/复制 Buttons, 高级查看 CheckBox — all operable by ref. The character grid itself is a custom-drawn control (coord/OCR), but every actionable control is by ref. Already-present system app. |
| 55 | Windows 系统信息 (msinfo32, hwnd 4789100) | Win32 Tree+List (#32770) UIA | 2026-08-01 | ✅ category Tree + system-data List | ✅ tree/list by ref | det (UIA) | **SUPPORTED — extract-real-valuable-data.** UIA exposes the category **Tree** (系统摘要/硬件资源/组件/软件环境) by ref and the **List** with Header (项目/值) + ListItems of **real system data** by ref — 操作系统名称/版本/操作系统制造商/系统名称/系统制造商/系统型号… Two Win32 common controls (TreeView + ListView) fully exposed. Already-present system app. |
| 56 | Windows DirectX 诊断工具 (dxdiag, hwnd 2036866) | Win32 tabbed (#32770) UIA | 2026-08-01 | ✅ tabs + labeled system data | ✅ tabs/buttons by ref | det (UIA) | **SUPPORTED — extract-real-valuable-data.** UIA exposes the Tab control (系统/显示/声音/输入) by ref, the 系统信息 Group with labeled fields (操作系统/系统制造商/系统型号/处理器/内存/DirectX 版本) — **"DirectX 12" value read by ref (e26)** — and Buttons 帮助/下一页/保存所有信息/退出. Win32 tabbed diagnostic dialog fully operable. Already-present system app. **This is the 50th distinct app evaluated in the program.** |
