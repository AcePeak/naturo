...
def wait_cmd(ctx: click.Context, selector: str, by: Optional[str],
    ...
    try:
        ...
        if json_output:
            click.echo(json_dumps({"success": True, "selector": selector, "state": state}))
        else:
...
def wait_navigation_cmd(ctx: click.Context, timeout: float,
    ...
    try:
        ...
        if json_output:
            click.echo(json_dumps({"success": True, "url": new_url}))
        else:
...
def wait_url_cmd(ctx: click.Context, pattern: str, regex: bool,
    ...
    try:
        ...
        if json_output:
            click.echo(json_dumps({"success": True, "url": matched_url}))
        else:
...
def wait_function_cmd(ctx: click.Context, expression: str,
    ...
    try:
        ...
        if json_output:
            click.echo(json_dumps({"success": True, "result": str(result)}))
        else:
...
def wait_network_idle_cmd(ctx: click.Context, idle_time: float,
    ...
    try:
        ...
        if json_output:
            click.echo(json_dumps({"success": True}))
        else:
