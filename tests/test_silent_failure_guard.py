...
def test_backend_exception_reports_failure(tool, args, method):
    ...
    # type_text now runs keystroke-first (naturo/actions.py:smart_type_text) — paste
    # is only a verified fallback and never pre-empts the keystroke — so
    # backend.type_text is reached directly and its RuntimeError must surface.
    with p1, p2:
